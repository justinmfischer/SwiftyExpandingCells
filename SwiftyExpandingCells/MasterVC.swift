...
class MasterVC: UITableViewController, UINavigationControllerDelegate, SegueHandlerType {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        let count = BrandManager.sharedInstance.brands.count
        return count
    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        self.selectedCellFrame = tableView.convert(tableView.cellForRow(at: indexPath as IndexPath)!.frame, to: tableView.superview)
        self.selectedBrand = BrandManager.sharedInstance.brands[indexPath.row]
        
        self.performSegueWithIdentifier(segueIdentifier: .DetailVC , sender: nil)
    }
